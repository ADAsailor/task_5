...
with sl.connect("space.db") as db:
    ...
    data2 = [
        ...
        ('Савицкая Светлана Евгеньевна', 74, 2),
    ...
    data3 = [
        (123, 2, 4, 'Высадка на Венеру', '17 часов'),
    ...
    print(alldata)
    cursor.execute("SELECT id, name FROM astronauts WHERE number_of_flights < 2")
    alldata = cursor.fetchall()
    print(alldata)
    cursor.execute("SELECT * FROM planets WHERE name LIKE '%Юпитер%'")
    alldata = cursor.fetchall()
    print(alldata)
    db.commit()
db.close()
